...
async def update_balance_history_for_date(
    db: AsyncSession,
    user_id,
    ...
    previous_balance_result = await db.execute(
        ...
        .where(
            BalanceHistory.user_id == user_uuid,
            BalanceHistory.date == previous_day
        )
...
async def recalculate_balance_history(
    ...
    if recalc_data.from_date:
        ...
        earliest_income_result = await db.execute(
            select(func.min(Income.date)).where(Income.user_id == current_user.id)
        )
        earliest_income = earliest_income_result.scalar_one()
        
        earliest_expense_result = await db.execute(
            select(func.min(Expense.date)).where(Expense.user_id == current_user.id)
        )
    ...
    income_dates_result = await db.execute(
        select(func.distinct(Income.date))
        .where(
    ...
    expense_dates_result = await db.execute(
        select(func.distinct(Expense.date))
        .where(
