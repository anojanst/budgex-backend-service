...
from app.schemas.tag import TagCreate, TagResponse, TagUpdate, TagWithStats
...
async def list_tags(
    ...
    if budget_id:
        ...
        expense_tags_query = select(distinct(Expense.tag_id)).where(Expense.budget_id == budget_id, Expense.tag_id.isnot(None))
    ...
    if used_with == "expenses":
        expense_tags_query = select(distinct(Expense.tag_id)).where(
            Expense.tag_id.isnot(None), Expense.user_id == current_user.id
        )
        expense_tags_result = await db.execute(expense_tags_query)
        expense_tag_ids = [tag_id for tag_id, in expense_tags_result.all() if tag_id]
        query = query.where(Tag.id.in_(expense_tag_ids))
    elif used_with == "incomes":
        income_tags_query = select(distinct(Income.tag_id)).where(Income.tag_id.isnot(None), Income.user_id == current_user.id)
...
async def get_tag(
    ...
    expense_stats_result = await db.execute(
        ...
        ).where(Expense.tag_id == tag_id, Expense.user_id == current_user.id)
    )
    ...
    income_stats_result = await db.execute(
        ...
        ).where(Income.tag_id == tag_id, Income.user_id == current_user.id)
    )
...
async def create_tag(
    ...
    existing_result = await db.execute(select(Tag).where(Tag.user_id == current_user.id, Tag.name == tag_data.name))
...
async def delete_tag(
    ...
    expense_count_result = await db.execute(select(func.count(Expense.id)).where(Expense.tag_id == tag_id))
    expense_count = expense_count_result.scalar_one()

    income_count_result = await db.execute(select(func.count(Income.id)).where(Income.tag_id == tag_id))
